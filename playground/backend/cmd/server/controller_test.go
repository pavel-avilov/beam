...
func Test_playgroundController_GetCompileOutput(t *testing.T) {
	ctx := context.Background()
	conn, err := grpc.DialContext(ctx, "bufnet", grpc.WithContextDialer(bufDialer), grpc.WithInsecure())
	if err != nil {
		t.Fatalf("Failed to dial bufnet: %v", err)
	...
	tests := []struct {
		...
		{
			name: "GetСompileOutput with right pipelineId",
			args: args{
				ctx:  ctx,
				info: &pb.GetCompileOutputRequest{PipelineUuid: pipelineId},
			},
			want:    &pb.GetCompileOutputResponse{Output: ""},
			wantErr: false,
		},
		{
			name: "GetCompileOutput with wrong pipelineId",
			args: args{
				ctx:  ctx,
				info: &pb.GetCompileOutputRequest{PipelineUuid: wrongUuid},
			},
			wantErr: true,
		},
	}
