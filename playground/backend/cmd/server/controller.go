...
func (controller *playgroundController) RunCode(ctx context.Context, info *pb.RunCodeRequest) (*pb.RunCodeResponse, error) {
	...
	go processCode(context.TODO(), controller.cacheService, lc, exec, pipelineId, pipelineExecuteTimeout, controller.env)
...
func processCode(ctx context.Context, cacheService cache.Cache, lc *fs_tool.LifeCycle, execBuilder *executors.CompileBuilder, pipelineId uuid.UUID, timeout time.Duration, env *environment.Environment) {
	...
	setToCache(ctxWithTimeout, cacheService, pipelineId, cache.CompileOutput, "")
